...
| Migrated functional | `playwright.config.ts`, project `functional` | `yarn test:playwright:functional` | 11 behaviour tests across the explicit `zoom`, `navigation`, `rotation` and `search` feature files. See [`playwright_tests/functional/README.md`](playwright_tests/functional/README.md). |
