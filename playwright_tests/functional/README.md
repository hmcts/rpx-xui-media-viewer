...
| **Functional total** | 13 feature files | **83 default / 85 with EXUI-5124 opt-in** | Matches current Playwright functional discovery. The two tagged image-create/draw contracts remain discoverable but are excluded from default execution while the known product defect is resolved. |
| **Playwright migration total** | Functional plus smoke | **84 default / 86 with EXUI-5124 opt-in** | Matches functional discovery plus one standalone PDF-readiness contract. External AAT/CCD diagnostics are intentionally excluded from normal migration assurance. Support checks are reported separately. |
