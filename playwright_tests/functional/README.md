...
| PDF annotations | `annotations.spec.ts` | 7 | Real PDF text selection, distinct two-marker draw-box geometry through reload, selected-highlight comments through rotation/reload, two-comment collation, every-result search annotation and individual DELETE requests for every existing highlight. The live annotation-service boundary is separately covered by Integration. |
| Image annotations and comments | `annotations.spec.ts`, `integration/aatCcdDmStore.spec.ts` | 5 | Four real-browser Functional contracts map the historical create, draw-box, update and delete behaviours one-for-one. Update and delete run by default; create and draw-box remain discoverable but excluded by [EXUI-5124](https://tools.hmcts.net/jira/browse/EXUI-5124), because the current product does not persist an image draw-box annotation. The default Integration contract proves the live annotation-set create/retrieve/update/delete lifecycle after a DM Store upload. |
...
| **Playwright migration total** | Functional plus smoke and Integration | **79 discovered / 77 default** | Adds one standalone PDF-readiness contract and five live Integration contracts. Support checks are reported separately. |
