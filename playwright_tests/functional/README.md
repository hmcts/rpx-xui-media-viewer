...
| **Functional total** | 13 feature files | **76 discovered / 74 default** | Behaviour-level Functional coverage; the two non-default contracts are ticketed product defects, not skipped tests. |
| **Playwright migration total** | Functional plus smoke | **77 discovered / 75 default** | Adds one standalone PDF-readiness contract. External service diagnostics are intentionally excluded from normal migration assurance. Support checks are reported separately. |
