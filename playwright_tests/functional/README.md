...
PDF-readiness contract. The full product capability inventory and next assurance
gaps are stored in
...
| Bookmarks | `bookmarks.spec.ts` | 12 | Playwright covers highlight-origin and panel creation, update, delete/sibling promotion, single and multiple empty drafts, sorting, rendered drag-reorder persistence, and bulk lifecycle. |
...
The capability report uses `Capability status` and `Default execution` for
normal CI selection. The opt-in external-service contracts and the two
