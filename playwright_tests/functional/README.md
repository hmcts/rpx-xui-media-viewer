...
| Office document conversion | `officeConversion.spec.ts` | 9 | Five supported-format conversion requests (Word, Excel, PowerPoint, text and RTF), returned-PDF rendering, HTTP 400/500 and timeout non-rendering/form recovery, and malformed-PDF handling using deterministic document-assembly responses. |
...
| **Functional total** | 13 feature files | **82 discovered / 80 default** | Behaviour-level Functional coverage; the two image draw contracts remain ticketed product defects and are not counted as default execution on this branch. |
| **Playwright migration total** | Functional plus smoke | **83 discovered / 81 default** | Adds one standalone PDF-readiness contract. External service diagnostics are intentionally excluded from normal migration assurance. Support checks are reported separately. |
