...
| Image annotations and comments | `annotations.spec.ts` | 4 | One deterministic Functional annotation-service contract per historical Codecept scenario: image annotation creation, positive persisted geometry, comment create/update/delete. Integration separately creates and retrieves a live annotation set for a newly uploaded DM Store image. The historical Codecept interaction was synthetic; the image overlay is not actionable in a real browser. |
...
| **Functional total** | 12 feature files | **73** | Behaviour-level functional coverage |
| **Playwright total** | Functional plus smoke and Integration | **79** | Adds one standalone PDF-readiness contract and five live Integration contracts |
