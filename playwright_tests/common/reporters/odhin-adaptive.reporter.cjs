...
class OdhinAdaptiveReporter {
  constructor(options = {}) {
    ...
    this.trimFailedArtifacts = normalizeBoolean(reporterOptions.trimFailedArtifacts ?? process.env.PW_ODHIN_TRIM_FAILED_ARTIFACTS, false);
    this.statusCounts = {
      passed: 0,
      failed: 0,
      skipped: 0,
  ...
  recordFeatureStat(test, result) {
    ...
    const featureName = deriveFeatureName(test?.location?.file, test?.tags);
